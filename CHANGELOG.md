### 第52次发布

版本号 1.0.68

本次发布包含了以下内容：

1. 新增实时识别 V2（RealtimeRecognizerV2），句子模式，话者分离作为可选开关，支持扩展参数
### 第51次发布

版本号 1.0.67

本次发布包含了以下内容：

1. 添加句子模式单独支持
### 第50次发布

版本号 1.0.66

本次发布包含了以下内容：

1. 添加句子 & 段落模式支持
### 第49次发布

版本号 1.0.65

本次发布包含了以下内容：

1. 增加语音翻译
### 第48次发布

版本号 1.0.64

本次发布包含了以下内容：

1. 添加上下文能力
### 第47次发布

版本号 1.0.63

本次发布包含了以下内容：

1. 交互式播客支持：大模型播客增加参数 ContextId、EnableWebSearch，返回结果中增加 context_id 和 usage_tokens 
2. 大模型播客添加 cancel 方法主动取消合成
### 第46次发布

版本号 1.0.62

本次发布包含了以下内容：

1. 录音识别极速版支持指定替换词
### 第45次发布

版本号 1.0.61

本次发布包含了以下内容：

1. 更新 okhttp 到 4.12.0
### 第44次发布

版本号 1.0.60

本次发布包含了以下内容：

1. 新增大模型播客sdk
### 第43次发布

版本号 1.0.59

本次发布包含了以下内容：

1. 修复flash
### 第42次发布

版本号 1.0.57

本次发布包含了以下内容：

1. 修复重试逻辑
### 第41次发布

版本号 1.0.56

本次发布包含了以下内容：

1. 支持热词不编码
### 第40次发布

版本号 1.0.54

本次发布包含了以下内容：

1. 修复soe录音识别问题
### 第40次发布

版本号 1.0.53

本次发布包含了以下内容：

1. soe添加参数rec_mode
### 第40次发布

版本号 1.0.52

本次发布包含了以下内容：

1. 优化极速版识别
### 第39次发布

版本号 1.0.51

本次发布包含了以下内容：

1. 支持代理配置proxyProfile
### 第38次发布

版本号 1.0.50

本次发布包含了以下内容：

1. 支持代理配置proxyProfile
### 第37次发布

版本号 1.0.49

本次发布包含了以下内容：

1. 流式识别修复若干问题
### 第36次发布

版本号 1.0.48

本次发布包含了以下内容：

1. 支持声音变换（websocket）
### 第36次发布

版本号 1.0.47

本次发布包含了以下内容：

1. 支持流式合成
### 第36次发布

版本号 1.0.46

本次发布包含了以下内容：

1. SpeechClient添加maxFramePayloadLength配置
### 第35次发布

版本号 1.0.45

本次发布包含了以下内容：

1. fix close方法conn为空的情况
### 第34次发布

版本号 1.0.44

本次发布包含了以下内容：

1. 调整TtsConstant.DEFAULT_START_TIMEOUT_MILLISECONDS 为60秒
### 第33次发布

版本号 1.0.43

本次发布包含了以下内容：

1. 支持soe
### 第32次发布

版本号 1.0.41

本次发布包含了以下内容：

1. asr 实时识别框架重构
### 第31次发布

版本号 1.0.40

本次发布包含了以下内容：

1. tts ws 优化默认配置
### 第30次发布

版本号 1.0.39

本次发布包含了以下内容：

1. tts ws 时间戳鉴权问题

### 第29次发布

版本号 1.0.38

本次发布包含了以下内容：

1. asr ws支持max_speak_time
2. asr flash 修复hotword_list参数报错问题
### 第28次发布

版本号 1.0.36

本次发布包含了以下内容：

1. asr ws支持
### 第27次发布

版本号 1.0.35

本次发布包含了以下内容：

1. fix tts初始化配置问题
### 第26次发布

版本号 1.0.34

本次发布包含了以下内容：

1. 虚拟号start方法添加返回值
### 第25次发布

版本号 1.0.33

本次发布包含了以下内容：

1. 修复虚拟号VirtualNumberResponse 注解
### 第24次发布

版本号 1.0.32

本次发布包含了以下内容：

1. 修复TTS websocket 鉴权问题
### 第23次发布

版本号 1.0.30 

本次发布包含了以下内容：

1. 支持asr websocket 支持配置多个账号

### 第22次发布

版本号 1.0.29 

本次发布包含了以下内容：

1. 支持tts websocket

### 第21次发布

版本号 1.0.28 

本次发布包含了以下内容：

1. 支持私有化asr


### 第20次发布

版本号 1.0.27 

本次发布包含了以下内容：

1. 支持虚拟号功能添加waitTime参数

### 第19次发布

版本号 1.0.25 

本次发布包含了以下内容：

1. 支持虚拟号功能
2. 实时识别和录音识别极速版添加新参数
### 第18次发布

版本号 1.0.23 

本次发布包含了以下内容：

1. 实时识别添加新参数noiseThreshold
2. 修复特殊场景tts鉴权失败问题
### 第17次发布

版本号 1.0.22 

本次发布包含了以下内容：

1. 实时识别支持自学习模型
### 第16次发布

版本号 1.0.20 

本次发布包含了以下内容：

1. 优化错误信息提示。

### 第15次发布

版本号 1.0.19 

本次发布包含了以下内容：

1. 优化错误信息提示。
2. 语音合成支持自定义sessionid
3. 支持配置代理

### 第14次发布

版本号 1.0.18 

本次发布包含了以下内容：

1. 优化websocket识别，添加配置类SpeechWebsocketConfig。
2. 添加日志拦截器SdkLogInterceptor。
3. 优化异常处理，添加SdkRunException。
5. websocket request 支持配置voiceId参数。
6. 优化http请求参数，修复http实时识别问题。

### 第13次发布

版本号 1.0.17 

本次发布包含了以下内容：

1. 添加ws请求配置项，可在SpeechRecognitionSysConfig配置 。
2. 添加ws listener回调方法onMessage

### 第12次发布

版本号 1.0.15 /1.0.16

本次发布包含了以下内容：

1. 优化。

### 第11次发布

版本号 1.0.14 

本次发布包含了以下内容：

1. 修复flash 返回值错误问题。
### 第10次发布

版本号 1.0.13 

本次发布包含了以下内容：

1. asr 添加voice_id返回值。

### 第9次发布

版本号 1.0.12 

本次发布包含了以下内容：

1. 添加录音识别极速版功能。

### 第8次发布

版本号 1.0.11 

本次发布包含了以下内容：

1. 修改tts偶发鉴权问题。

### 第7次发布

版本号 1.0.8 

本次发布包含了以下内容：

1. 修改已知bug。

### 第7次发布

版本号 1.0.7 

本次发布包含了以下内容：

1. 修改已知bug。

### 第6次发布

版本号 1.0.6 

本次发布包含了以下内容：

1. 修改已知问题。


### 第5次发布

版本号 1.0.5 

本次发布包含了以下内容：

1. 修改已知问题。
2. 规范文档描述。

### 第4次发布

版本号 1.0.4 

本次发布包含了以下内容：

1. 修改已知问题。
2. 规范文档描述。
3. asr 修改调用方法名，原有的 recognize修改为write ,end修改为stop ,添加start方法。
4. asr 1.0.4版本方法调用顺序由recognize->end修改为 start->write->stop。
5. tts 修改类名，原有前缀为SpeechSynthesizer 修改为SpeechSynthesis。
6. asr添加websocket的实现方式。
7. json解析框架fastjson替换为jackson。
8. asr http添加同步实现方式可通过SpeechRecognitionSysConfig.ifSyncHttp=true; 指定。

### 第3次发布

版本号 1.0.3 

本次发布包含了以下内容：

1. TTS  SpeechSynthesizerListener回调添加cancel方法，用户可取消请求。
2. 规范文档。

### 第2次发布

版本号 1.0.2
本次发布包含了以下内容：

1. 规范调用方式以及命名方式。具体调用方式可参考案例。
2. 支持TTS SDK .
3. 优化ASR SDK 调用方式。

### 第1次发布
版本号 1.0.1

发布时间：2020-09-12 08:22:02
本次发布包含了以下内容：

初始化语音识别SDK代码。




